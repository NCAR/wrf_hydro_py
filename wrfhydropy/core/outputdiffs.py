...
def compare_ncfiles(candidate_files: list,
    ...
    ref_dir = pathlib.Path(reference_files[0]).parent

    output_list = []
    for file_candidate in candidate_files:
        file_candidate = pathlib.Path(file_candidate)

        file_reference = pathlib.Path(ref_dir.joinpath(file_candidate.name))
