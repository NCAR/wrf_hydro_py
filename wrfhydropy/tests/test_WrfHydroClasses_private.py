...
# test_data_dir = pathlib.Path('/home/docker/wrf_hydro_py/wrfhydropy/tests/data/private')
...
def test_domain_nwm(tmp_data_dir, capsys):
    with capsys.disabled():
        print(
            "\nQuestion: The WrfHydroDomain class is constructed properly for NWM private? ",
            end=''
        )
...
# wrf_hydro_nwm_dir = test_data_dir / 'wrf_hydro_nwm'
...
def test_model_nwm(tmp_data_dir, capsys):
    with capsys.disabled():
        print(
            "\nQuestion: WrfHydroModel object is able to compile NWM private? ",
            end=''
        )
...
def test_setup_nwm(tmp_data_dir, capsys):
    with capsys.disabled():
        print(
            "\nQuestion: WrfHydroSetup object is constructed properly for NWM private? ",
            end=''
        )
...
def test_job_list_nwm(tmp_data_dir, capsys):
    with capsys.disabled():
        print(
            "\nQuestion: Job object still the same on NWM private? ",
            end=''
        )
...
def test_run_nwm(tmp_data_dir, capsys):
    with capsys.disabled():
        print(
            "\nQuestion: WrfHydroSetup object is able to run NWM private? ",
            end=''
        )
